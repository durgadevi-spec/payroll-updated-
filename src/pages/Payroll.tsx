...
const getNetSalary = (item: PayrollItemWithEmployee) => {
  ...
  return Math.max(0, Math.round((monthlySalary - leaveDeduction - tsDeduction - mpDeduction - pfDeduction - esiDeduction - taxDeduction - loanDeduction - advanceDeduction - permissionDeduction + bonus + sundayEarnings) * 100) / 100);
};
  ...
  async function generatePayroll(dataOverride?: any) {
    ...
    try {
      ...
      for (const emp of dataToUse.employees) {
        ...
        const unpaidLeaves = Math.max(0, emp.summary.unpaidDays - punchMissing); // Subtract missing punches since unpaidDays lumps them all, and we want it separate
        const missingTimesheets = 0;
        const sundayDeductions = emp.summary.sundayDeductions;
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-100 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                    {['Employee', 'Leave Taken', 'Leave Source', 'Timesheet Status', 'Missing TS', 'TS Detection', 'Missing Punch', 'Punch Ded.', 'Permissions', 'Perm. Ded.', 'Holidays', 'Advance', 'Sunday Work', 'Net Salary', 'Actions'].map(h => (
    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-slate-100 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
              {['Employee', 'Mode', 'Days', 'Monthly Sal.', 'Leave Taken', 'Leave Ded.', 'Missing TS', 'TS Ded.', 'Missing Punch', 'Punch Ded.', 'Permissions', 'Perm. Ded.', 'Holidays', 'Advance', 'Sunday', 'PF', 'ESI', 'Tax', 'Bonus', 'Net Salary', ''].map(h => (
                <th key={h} className="py-2 px-3 text-left font-semibold text-slate-600 dark:text-slate-300 whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
              ...
              return (
                <React.Fragment key={item.id}>
                  <tr className="border-b border-slate-50 dark:border-slate-700/30 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="py-2 px-3">
                      <div className="flex flex-col">
                        ...
                        {(() => {
                          ...
                          const normalCovered = coveredDates.length - odCovered;
                          return (
                            <div className="text-[9px] text-green-600 dark:text-green-400 mt-0.5 whitespace-nowrap">
                              {odCovered > 0 && <div>{odCovered} approved OD no deduction</div>}
                              {normalCovered > 0 && <div>{normalCovered} covered by leave</div>}
                            </div>
                          );
                    ...
                    <td className="py-2 px-3 text-red-500">-{formatCurrency(getMissingPunchDeduction(item))}</td>
                    <td className="py-2 px-3 font-medium text-slate-700 dark:text-slate-300">{item.permission_hours || 0}h</td>
                    <td className="py-2 px-3 text-red-500">-{formatCurrency(item.permission_deduction || 0)}</td>
                    <td className="py-2 px-3">
                      ...
                      <td colSpan={21} className="px-4 py-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-[11px]">
                          ...
                          <div className="bg-orange-50 dark:bg-orange-900/20 p-2 rounded border border-orange-200 dark:border-orange-800">
                            <p className="font-semibold text-orange-600 dark:text-orange-400 mb-1">🔴 Missing Punches ({((item as any).missing_punches || 0) + ((item as any).covered_by_leave_dates || []).length} days)</p>
                            <p className="text-[9px] text-slate-400 mb-1">No biometric punch & no LMS leave → salary deducted</p>
                            {((item as any).missing_punch_dates || []).length > 0
                              ? ((item as any).missing_punch_dates as string[]).map((d: string) => <div key={d} className="text-orange-700 dark:text-orange-300">{fmt(d)}</div>)
                                ...
                                <div className="mt-1 pt-1 border-t border-orange-200 dark:border-orange-700">
                                  {odCoveredDates.length > 0 && (
                                    <>
                                      ...
                                      {odCoveredDates.map((d: string) => <div key={d} className="text-green-700 dark:text-green-300">{fmt(d)} ✓</div>)}
                                    </>
                                  )}
                                  {normalCoveredDates.length > 0 && (
                                    <>
                                      <p className="text-[9px] text-green-600 dark:text-green-400 font-medium mt-1 mb-0.5">✓ Covered by leave (no deduction):</p>
                                      {normalCoveredDates.map((d: string) => <div key={d} className="text-green-700 dark:text-green-300">{fmt(d)} ✓</div>)}
                                    </>
                                  )}
                                </div>
                              );
