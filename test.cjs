...
async function run() {
  try {
    ...
    const attRes = await pool.query(
      "SELECT DISTINCT TO_CHAR(punch_time AT TIME ZONE 'Asia/Kolkata', 'YYYY-MM-DD') as att_date FROM attendance_logs WHERE emp_code = '" + emp.employee_code + "' ORDER BY att_date"
    );
